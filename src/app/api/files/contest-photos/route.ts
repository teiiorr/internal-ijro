...
export async function POST(req: NextRequest) {
  ...
  const caption = (url.searchParams.get("caption") ?? "").trim().slice(0, 255);
  ...
  try {
    stored = await storeStream(req.body, {
      fileName: name,
      subdir: `contest-photos/${contestId}`,
  ...
  await db.insert(contestPhotos).values({
    contestId,
    fileUrl: stored.url,
  ...
  await logActivity({ userId: session.user.id, action: "contest.photo_added", entityType: "contest", entityId: contestId, newValue: { fileName: stored.originalName } });
