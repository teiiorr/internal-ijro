  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-4">
        {[
          { label: "Active employees", value: counts.total, href: "/employees" },
          { label: "New this month", value: counts.newThisMonth, href: "/employees" },
          { label: "On leave now", value: counts.onLeaveNow, href: "/employees" },
          { label: "Pending invitations", value: counts.pending, href: "/employees?status=pending" },
        ].map((c) => (
          <Card key={c.label}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader><CardTitle>Birthdays this week</CardTitle></CardHeader>
          <CardContent>
            {birthdays.length === 0 ? (
              <p className="text-sm text-[var(--muted)]">No birthdays this week.</p>
            ) : (
        ...
        <Card>
          ...
          <CardContent>
            <ul className="space-y-1 text-sm">
              ...
              {depts.length === 0 && <p className="text-sm text-[var(--muted)]">No departments yet.</p>}
            </ul>
          </CardContent>
        </Card>
      </div>
